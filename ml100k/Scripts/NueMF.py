...
'''
def NDCGModel(ndcgBatchCompined,ndcgBatch,modelphase):
  #############
  l11=tf.layers.dense(inputs=ndcgBatchCompined, units=20,activation=tf.nn.relu, kernel_initializer=tf.random_normal_initializer(stddev=0.01))
  l12=tf.layers.dense(inputs=ndcgBatchCompined, units=20,activation=tf.nn.relu, kernel_initializer=tf.random_normal_initializer(stddev=0.01))
  ##
  b21=tf.reshape(ndcgBatch, [-1, 2])
  l21=tf.layers.dense(inputs=b21, units=20,activation=tf.nn.relu, kernel_initializer=tf.random_normal_initializer(stddev=0.01))
  #l21=tf.layers.dropout(l21,rate=0.3,training=phase)
  b22=tf.reshape(l21, [-1,10, 20])
  b22=tf.map_fn(my_elementwise_func,  b22)
  b23=tf.reduce_mean(b22,axis=1)
  l22=tf.layers.dense(inputs=b23, units=20,activation=tf.nn.relu, kernel_initializer=tf.random_normal_initializer(stddev=0.01))
  #l22=tf.layers.dropout(l22,rate=0.1,training=phase)
  ##
  l31=tf.multiply(l22, l12)

  ##
  l4=tf.layers.dense(inputs=l31, units=5,activation=tf.nn.relu, kernel_initializer=tf.random_normal_initializer(stddev=0.01))
  #l2=tf.layers.dropout(l2,rate=0.1,training=phase)
  ndcg=tf.layers.dense(inputs=l4, units=1,activation=tf.nn.sigmoid, kernel_initializer=tf.random_normal_initializer(stddev=0.01))
  return ndcg
'''
def svd(train,ItemData=False,UserData=False,Graph=True,lr=0.00003,ureg=0.0,ireg=0.0):
    ...
    #AdjacencyUsers = preprocessing.Normalizer().fit_transform(AdjacencyUsers)
    #AdjacencyItems = preprocessing.Normalizer().fit_transform(AdjacencyItems) 
    if(Graph):
    ...
    #UserFeatures= np.concatenate((np.identity(USER_NUM), AdjacencyUsers), axis=1) 
    #ItemFeatures= np.concatenate((np.identity(ITEM_NUM), AdjacencyItems), axis=1) 
 
    #UserFeatures= normalized_laplacian_matrix(np.concatenate((np.identity(USER_NUM), AdjacencyUsers), axis=1))
    #ItemFeatures= normalized_laplacian_matrix(np.concatenate((np.identity(ITEM_NUM), AdjacencyItems), axis=1)) 
    ...
    #def optLoss(NDCGscore,NDCGTrue, learning_rate=0.00003, reg=0.1, device="/cpu:0"):
    #def optNDCG(NDCGscore, learning_rate=0.00003, reg=0.1, device="/cpu:0"):
...
def GetTrainSample(DictUsers,BatchSize=1000,topn=10):
  ...
  for i in range(numusers):
    ...
    trainusers.extend(np.repeat(batchusers, topn))
    ##Pos
    posindexes=np.random.randint(len(DictUsers[batchusers][0]), size=5)
    allpositems=np.asarray(DictUsers[batchusers][0])
    trainitems.extend(np.take(allpositems, posindexes))

    allpositemsrates=np.asarray(DictUsers[batchusers][4])
    #print(allpositemsrates)
    traintargets.extend(np.take(allpositemsrates, posindexes))
    ##Neg
    
    negindexes=np.random.randint(len(DictUsers[batchusers][1]), size=5)
    allnegitems=np.asarray(DictUsers[batchusers][1])
    trainitems.extend(np.take(allnegitems, negindexes))
    traintargets.extend(np.zeros(int(5))) 
...
'''
'''
dictUsers,df_train=get_dataPos1M()
save_data(dictUsers,"../Data/ml1m/UserDict2.dat")
save_data(df_train,"../Data/ml1m/RankData2.dat")
'''
...
# Best 5:5 NDCG with rate,0.827610,3.035961,0.637328,0.285095,1.000000,0.607120,0.459964,0.356187
warnings.filterwarnings('ignore')
#svd(df_train,ItemData=True,UserData=True,Graph=True,lr=0.00007,ureg=0.001,ireg=0.001)
#svd(df_train,ItemData=UseItemData,UserData=UseUserData,Graph=UseGraphData,lr=0.000001,ureg=UserReg,ireg=ItemReg)
svd(df_train,ItemData=False,UserData=False,Graph=False,lr=0.001,ureg=0.0,ireg=0.0)
tf.reset_default_graph() 
